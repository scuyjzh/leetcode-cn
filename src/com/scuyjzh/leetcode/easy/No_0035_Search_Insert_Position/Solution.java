...
class Solution {
    ...
    public int searchInsert(int[] nums, int target) {
         ...
         * 在 O(log n) 的时间内找到是否存在目标值。但这题还多了个额外的条件，即如果不存在数组中的时候需
         * 要返回按顺序插入的位置，那还能用二分法么？答案是可以的，只需要稍作修改即可。
         *
         * 考虑这个插入的位置 pos，它成立的条件为：
         *     nums[pos−1] < target ≤ nums[pos]
         * 其中 nums 代表排序数组。由于如果存在这个目标值，返回的索引也是 pos，因此可以将两个条件合并得
         * 出最后的目标：「在一个有序数组中找第一个大于等于 target 的下标」。
         *
         * 问题转化到这里，直接套用二分法即可，即不断用二分法逼近查找第一个大于等于 target 的下标 。下文
         * 给出的代码是笔者习惯的二分写法，ans 初值设置为数组长度可以省略边界条件的判断，因为存在一种情况
         * 是 target 大于数组中的所有数，此时需要插入到数组长度的位置。
         */
        int len = nums.length;
