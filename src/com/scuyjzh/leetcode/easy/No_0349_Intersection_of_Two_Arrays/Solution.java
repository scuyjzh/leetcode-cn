...
class Solution {
    ...
    public int[] intersection1(int[] nums1, int[] nums2) {
        /*
         * 计算两个数组的交集，直观的方法是遍历数组 nums1，对于其中的每个元素，遍历数组
         * nums2 判断该元素是否在数组 nums2 中，如果存在，则将该元素添加到返回值。假设
         * 数组 nums1 和 nums2 的长度分别是 mm 和 nn，则遍历数组 nums1 需要 O(m)
         * 的时间，判断 nums1 中的每个元素是否在数组 nums2 中需要 O(n) 的时间，因此总
         * 时间复杂度是 O(mn)。
         *
         * 如果使用哈希集合存储元素，则可以在 O(1)O(1) 的时间内判断一个元素是否在集合中，
         * 从而降低时间复杂度。
         *
         * 首先使用两个集合分别存储两个数组中的元素，然后遍历较小的集合，判断其中的每个元
         * 素是否在另一个集合中，如果元素也在另一个集合中，则将该元素添加到返回值。该方法
         * 的时间复杂度可以降低到 O(m+n)。
         */
