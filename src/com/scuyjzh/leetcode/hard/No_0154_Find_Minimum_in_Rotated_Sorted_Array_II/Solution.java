...
class Solution {
    ...
    public int findMin(int[] nums) {
         ...
         * 因此，可以根据这一条性质，通过二分查找的方法找出最小值。
         *
         * 在二分查找的每一步中，左边界为 left，右边界为 right，区间的中点为 mid，最小值就在该区间内。
         * 将中轴元素 nums[mid] 与右边界元素 nums[right] 进行比较，可能会有以下的三种情况：
         *   1.第一种情况是 nums[mid] < nums[right]。
         *     这说明 nums[mid] 是最小值右侧的元素，因此可以忽略二分查找区间的右半部分。
         *   2.第二种情况是 nums[mid] > nums[right]。
         *     这说明 nums[mid] 是最小值左侧的元素，因此可以忽略二分查找区间的左半部分。
         *   3.第三种情况是 nums[mid] = nums[right]。
         *     由于重复元素的存在，并不能确定 nums[mid] 究竟在最小值的左侧还是右侧，因此不能莽撞地忽略某一部分的元素。
         *     唯一可以知道的是，由于它们的值相同，所以无论 nums[right] 是不是最小值，都有一个它的「替代品」nums[mid]，
         *     因此可以忽略二分查找区间的右端点。
         *
         ...
         *   • nums[mid] > nums[right] 时，mid 所指一定不是最小值，因为一定比 nums[right] 大，所以 left = mid + 1；
         *   • nums[mid] < nums[right] 时，mid 所指可能是最小值，所以 right = mid；
         *
         * 三、时间/空间复杂度是多少？
         *   • 时间复杂度
         *     - 一般情况：与二分相同为 O(logN)
