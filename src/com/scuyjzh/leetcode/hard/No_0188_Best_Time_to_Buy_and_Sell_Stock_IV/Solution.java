...
class Solution {
     ...
     * 空间复杂度：O(n*min(n,k)) 或 O(min(n,k))，取决于使用二维数组还是一维数组进行动态规划。
     */
    public int maxProfit(int k, int[] prices) {
         ...
         *
         * 由于在所有的 n 天结束后，手上不持有股票对应的最大利润一定是严格优于手上持有股票对应的最大利润的，
         * 然而完成的交易数并不是越多越好（例如数组 prices 单调递减，不进行任何交易才是最优的），因此最终的答案即为 sell[n−1][0..k] 中的最大值。
         *
         * 三、状态初始化
         ...
         * 需要注意的是，sell[i][j] 的状态转移方程中包含 buy[i−1][j−1]，在 j=0 时其表示不合法的状态，因此在 j=0 时，无需对 sell[i][j] 进行转移，让其保持值为 0 即可。
         *
         * 最后需要注意的是，本题中 k 的最大值可以达到 10^9，然而这是毫无意义的，因为 n 天最多只能进行 ⌊n/2⌋ 笔交易，其中 ⌊x⌋ 表示对 x 向下取整。
