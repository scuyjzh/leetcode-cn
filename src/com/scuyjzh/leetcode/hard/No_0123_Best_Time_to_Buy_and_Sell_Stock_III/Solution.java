...
class Solution {
    ...
    public int maxProfit1(int[] prices) {
         ...
         *   1.对于状态 1 而言，在第 i 天可以不进行任何操作，保持不变，也可以在未进行任何操作的前提下
         *     以 prices[i] 的价格买入股票，那么状态 1 的状态转移方程即为：
         *         dp[i][1] = max(dp[i-1][1], dp[i-1][0] - prices[i]);
         *   2.对于状态 2 而言，在第 i 天可以不进行任何操作，保持不变，也可以在只进行过一次买操作的前提下
         *     以 prices[i] 的价格卖出股票，那么状态 2 的状态转移方程即为：
         *          dp[i][2] = max(dp[i-1][2], dp[i-1][1] + prices[i]);
         *   3.对于状态 3 而言，在第 i 天可以不进行任何操作，保持不变，也可以在完成了一笔交易的前提下
         *     以 prices[i] 的价格买入股票，那么状态 3 的状态转移方程即为：
         *         dp[i][3] = max(dp[i-1][3], dp[i-1][2] - prices[i]);
         *   4.对于状态 4 而言，在第 i 天可以不进行任何操作，保持不变，也可以在进行过第二次买操作的前提下
         *     以 prices[i] 的价格卖出股票，那么状态 4 的状态转移方程即为：
         *         dp[i][4] = max(dp[i-1][4], dp[i-1][3] + prices[i]);
         *
         * 三、状态初始化
         * 对于边界条件，考虑第 i=0 天时的四个状态：
         ...
         *
         * 在动态规划结束后，由于可以进行不超过两笔交易，因此最终的答案在 dp[n-1][0]、dp[n-1][2]、dp[n-1][4] 中，且为三者中
         * 的最大值。然而可以发现，由于在边界条件中 dp[0][2] 和 dp[0][4] 的值已经为 0，并且在状态转移的过程中
         * 维护的是最大值，因此 dp[n-1][2] 和 dp[n-1][4] 最终一定大于等于 0。同时，如果最优的情况对应的是恰好一笔交
         * 易，那么它也会因为在转移时允许在同一天买入并且卖出这一宽松的条件，从 dp[n-1][2] 转移至 dp[n-1][4]，因此最
         * 终的答案即为 dp[n-1][4]。
         */
        int n = prices.length;
        int[][] dp = new int[n][5];
