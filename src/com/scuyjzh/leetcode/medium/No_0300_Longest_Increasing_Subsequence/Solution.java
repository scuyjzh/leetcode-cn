...
class Solution {
    ...
    public int lengthOfLIS1(int[] nums) {
         ...
         *
         * 二、状态转移
         * 从小到大计算 dp 数组的值，在计算 dp[i] 之前，已经计算出 dp[0…i−1] 的值，则状态转移方程
         * 为：
         *                  dp[i] = max(dp[j])+1, 其中 0≤j<i 且 num[j]<num[i]
         *
         * 即考虑往 dp[0…i−1] 中最长的上升子序列后面再加一个 nums[i]。由于 dp[j] 代表 nums[0…j] 中以
    ...
    public int lengthOfLIS2(int[] nums) {
         ...
         *
         * 基于上面的贪心思路，维护一个数组 d[i] ，表示长度为 i 的最长上升子序列的末尾元素的最小值，用 len
         * 记录目前最长上升子序列的长度，起始时 len 为 1，d[1]=nums[0]。
         *
         * 同时可以注意到 d[i] 是关于 i 单调递增的。因为如果 d[j]≥d[i] 且 j<i，考虑从长度为 i 的最长上
         * 升子序列的末尾删除 i−j 个元素，那么这个序列长度变为 j ，且第 j 个元素 x（末尾元素）必然小于 d[i]，
         * 也就小于 d[j]。那么就找到了一个长度为 j 的最长上升子序列，并且末尾元素比 d[j] 小，从而产生了矛
         * 盾。因此数组 d 的单调性得证。
         *
         * 依次遍历数组 nums 中的每个元素，并更新数组 d 和 len 的值。如果 nums[i]>d[len] 则更新 len=
         * len+1，否则在 d[1…len]中找满足 d[i−1]<nums[j]<d[i] 的下标 i，并更新 d[i]=nums[j]。
         *
