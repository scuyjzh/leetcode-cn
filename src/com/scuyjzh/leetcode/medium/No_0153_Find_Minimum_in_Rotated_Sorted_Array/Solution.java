...
class Solution {
    ...
    public int findMin(int[] nums) {
         ...
         *   1.第一种情况是 nums[mid] < nums[right]。这说明 nums[mid] 是最小值右侧的元素，因此可以忽略二分查找区间的右半部分。
         *   2.第二种情况是 nums[mid] > nums[right]。这说明 nums[mid] 是最小值左侧的元素，因此可以忽略二分查找区间的左半部分。
         *   3.由于数组不包含重复元素，并且只要当前的区间长度不为 1，mid 就不会与 right 重合；而如果当前的区间长度为 1，这说明已经
         *     可以结束二分查找了。因此不会存在 nums[mid] = nums[right] 的情况。
         *
