...
class Solution {
    ...
    public int maxProfit1(int[] prices) {
         ...
         * 在初入股市时，只有「买入」的权利，只能获得负收益。而当「买入」之后，就有了「卖出」的权利，可以获得正收益。
         * 显然，需要尽可能地降低负收益而提高正收益，因此目标总是将收益值最大化。
         * 因此，可以使用动态规划的方法，维护在股市中每一天结束后可以获得的「累计最大收益」，并以此进行状态转移，得到最终的答案。
         *
         * 一、状态定义
         * 用 f[i] 表示第 i 天结束之后的「累计最大收益」。
         * 根据题目描述，由于最多只能同时买入（持有）一支股票，并且卖出股票后有冷冻期的限制，因此会有三种不同的状态：
         ...
         *
         * 二、状态转移
         * 在第 i 天时，可以在不违反规则的前提下进行「买入」或者「卖出」操作，此时第 i 天的状态会从第 i−1 天的状态转移而来；
         * 也可以不进行任何操作，此时第 i 天的状态就等同于第 i−1 天的状态。那么分别对这三种状态进行分析：
         ...
         *
         * • 对于 f[i][1]，在第 i 天结束之后处于冷冻期的原因是在当天卖出了股票，那么说明在第 i−1 天时必须持有一支股票，
         *   对应的状态为 f[i−1][0] 加上卖出股票的正收益 prices[i]。因此状态转移方程为：
         *       f[i][1] = f[i−1][0] + prices[i]
         *
         * • 对于 f[i][2]，在第 i 天结束之后不持有任何股票并且不处于冷冻期，说明当天没有进行任何操作，即第 i−1 天时不持有任何股票：
         *   如果处于冷冻期，对应的状态为 f[i−1][1]；如果不处于冷冻期，对应的状态为 f[i−1][2]。因此状态转移方程为：
         *       f[i][2] = max(f[i−1][1], f[i−1][2])
         *
         * 这样就得到了所有的状态转移方程。如果一共有 n 天，那么最终的答案即为 max(f[n−1][0], f[n−1][1], f[n−1][2])。
         * 注意到如果在最后一天（第 n−1 天）结束之后，手上仍然持有股票，那么显然是没有任何意义的。
         * 因此更加精确地，最终的答案实际上是 f[n−1][1] 和 f[n−1][2] 中的较大值，即 max(f[n−1][1], f[n−1][2])。
         *
         * 三、状态初始化
         * 可以将第 0 天的情况作为动态规划中的边界条件：
         * • 在第 0 天时，如果持有股票，那么只能是在第 0 天买入的，对应负收益 −prices[0]，即 f[0][0] = −prices[0]；
         * • 如果不持有股票，那么收益为零，即 f[0][1] = 0, f[0][2] = 0。
         * 注意到第 0 天实际上是不存在处于冷冻期的情况的，但仍然可以将对应的状态 f[0][1] 置为零，这其中的原因留给读者进行思考。
         *
     ...
     * 方法二：动态规划（空间优化）
     * 时间复杂度：O(n)，其中 n 为数组 prices 的长度。
     * 空间复杂度：O(1)。需要 3n 的空间存储动态规划中的所有状态，对应的空间复杂度为 O(n)。如果使用空间优化，空间复杂度可以优化至 O(1)。
     */
    public int maxProfit2(int[] prices) {
        /*
         * 空间优化：
         * 注意到上面的状态转移方程中，f[i][..] 只与 f[i−1][..] 有关，而与 f[i−2][..] 及之前的所有状态都无关，因此不必存储这些无关的状态。
         * 也就是说，只需要将 f[i−1][0]、f[i−1][1]、f[i−1][2] 存放在三个变量中，通过它们计算出 f[i][0]、f[i][1]、f[i][2] 并存回对应的变量，以便于第 i+1 天的状态转移即可。
         */
        if (prices.length == 0) {
            return 0;
        }
