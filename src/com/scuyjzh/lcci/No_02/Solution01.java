...
class Solution01 {
    /**
     * 方法一：哈希表
     * 时间复杂度：O(N)，其中 N 是给定链表中结点的数目
     * 空间复杂度：O(N)。在最坏情况下，给定链表中每个结点都不相同，哈希表中需要存储所有的 N 个值
     */
    public ListNode removeDuplicateNodes1(ListNode head) {
        ...
        while (pre.next != null) {
            ...
            if (occurred.add(cur.val)) {
                ...
                // 因此如果在面试中遇到了本题，可以和面试官确认是否需要释放被移除的结点占用的内存空间
    ...
    /**
     * 方法二：两重循环
     * 时间复杂度：O(N^2)，其中 N 是给定链表中结点的数目
     * 空间复杂度：O(1)
     */
    public ListNode removeDuplicateNodes2(ListNode head) {
        ListNode ob = head;
        // 第一重循环从链表的头结点开始，枚举一个保留的结点，这是因为我们保留的是「最开始出现的结点」
        while (ob != null) {
            ListNode oc = ob;
            // 第二重循环从枚举的保留结点开始，到链表的末尾结束，将所有与保留结点相同的结点全部移除
            while (oc.next != null) {
